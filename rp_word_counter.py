...
def _find_substring_indexes(desired_substring, message, index_type='start'): # returns index/es of substring
    array_of_indexes=[]
...
def _check_pair_overlap(main_pair, secondary_pair): # checks the overlap of two pairs
    debug_mode=False
...
def _remove_redundant_pairs(valid_pairs): # returns a pair without redundant pairs
    ...
    while True: # loop as long as there's things to remove
        ...
        while True: # loop through all the other pairs
            ...
            overlap=_check_pair_overlap(main_pair, secondary_pair) # see overlap
...
def count(msg):
    ...
    for i,indicator in enumerate(rp_indicators):
        indicator_indexes.append(_find_substring_indexes(indicator, message))
        # remove doubles of said characters
        for double in _find_substring_indexes(indicator+indicator, message, "all"):
    ...
    valid_pairs=_remove_redundant_pairs(valid_pairs)
